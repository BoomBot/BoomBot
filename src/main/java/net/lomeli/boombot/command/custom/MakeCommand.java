...
public class MakeCommand implements ICommand {
    ...
    public CommandResult execute(CommandData cmd) {
        if (Strings.isNullOrEmpty(cmd.getMessage()) || cmd.getArgs().size() < 2) return new CommandResult("boombot.command.make.empty");
        ...
        if (BoomBot.commandRegistry.getCommand(name) != null) return new CommandResult("boombot.command.make.name.used");
